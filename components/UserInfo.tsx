  ...
  return (
    <div className='flex flex-col  items-center'>
        <div className='relative w-fit'>
              <Image src={user?.avatar?.url || '/user.png'} alt='avatar' width={200} height={200} className='rounded-full border border-purple-950 shadow-md shadow-black bg-cover bg-center h-48 w-48 ' />
        ...
        <form onSubmit={handleInfo} className='flex flex-col gap-4 w-[60%] mt-4 font-normal'>
           <div>
                  <label htmlFor="name" className='text-slate-700 font-semibold'>User Name</label>
                  <Input type='text' id='name' value={name} onChange={(e) => setName(e.target.value)} placeholder='Name' className='mt-2 bg-slate-200 border shadow-sm shadow-black text-semibold text-slate-800 ' />
           </div>

              <div>
                  <label htmlFor="email" className='text-slate-700 font-semibold'>Email</label>
                  <Input type='text' id='email' value={user.email || "abc@example.com"} readOnly className='shadow-sm shadow-black mt-2 bg-slate-200 border text-slate-800 font-semibold ' />
              </div>
